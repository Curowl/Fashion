...
class HomeController extends Controller
{
    ...
    public function index() {
        ...
        foreach (SaleReturn::completed()->with('saleReturnDetails')->get() as $return) {
            foreach ($return->saleReturnDetails as $returnDetail) {
                if (!is_null($returnDetail->product)) {
                    $product_costs -= $returnDetail->product->product_cost * $returnDetail->quantity;
                }
            }
        }
    ...
    public function currentMonthChart() {
        ...
        $currentMonthSales = Sale::where('status', 'Completado')->whereMonth('date', date('m'))
                ->whereYear('date', date('Y'))
                ->sum('total_amount') / 100;
        $currentMonthPurchases = Purchase::where('status', 'Completado')->whereMonth('date', date('m'))
                ->whereYear('date', date('Y'))
                ->sum('total_amount') / 100;
    ...
    public function salesChartData() {
        ...
        $sales = Sale::where('status', 'Completado')
    ...
    public function purchasesChartData() {
        ...
        $purchases = Purchase::where('status', 'Completado')
            ->where('date', '>=', $date_range)
            ->groupBy(DB::raw("DATE_FORMAT(date,'%d-%m-%y')"))
            ->orderBy('date')
            ->get([
